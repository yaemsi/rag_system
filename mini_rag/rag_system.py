...
Pipeline per question:
  ...
  2. Hybrid retrieval (dense + BM25, RRF fusion) → top_k_final candidates
  3. Cross-encoder reranker → top_k_reader chunks          [optional]
...
from mini_rag.reranker import Reranker
...
class RAGQnASystem(QnASystem):
    ...
    def from_corpus(
        ...
        return cls(
            retriever=retriever,
            reranker=Reranker(rerank_params),
            reader=Reader(read_params),
        )
    ...
    def get_answers(self, questions: list[str]) -> list[GroundedAnswer | None]:
        ...
        return [
            self._answer_one(q)
            for q in tqdm(questions, desc="Generating answers", unit="q")
        ]
    ...
    def _answer_one(self, question: str) -> GroundedAnswer | None:
        ...
        # Step 2: retrieve top_k_final candidates
        ...
        # Step 3: rerank → top_k_reader chunks
        chunks = self._reranker.rerank(query=question, chunks=chunks)

        # Step 4: generate
        answer_text, doc_ids = self._reader.answer(
            question=question,
            chunks=chunks,
            low_confidence=chunks[0].score < self._reader._params.low_confidence_threshold,
        )
...
def build_system(
    ...
    if index_exists and not force_rebuild:
        ...
        return RAGQnASystem.load(ret_params, rerank_params, read_params, index_dir)

    logger.info(f"[build_system] Building new index from {corpus_path} …")
    docs = load_corpus(corpus_path)
    return RAGQnASystem.from_corpus(
        chunk_params, ret_params, rerank_params, read_params,
        docs, index_dir=index_dir,
    )
