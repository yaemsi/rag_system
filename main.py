...
def main() -> None:
    parser = HfArgumentParser(
        (GeneralArguments, ChunkerArguments, RetrieverArguments, RerankerArguments, ReaderArguments)
    )
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        gen_args, chunk_args, ret_args, rerank_args, read_args = parser.parse_json_file(
            json_file=os.path.abspath(sys.argv[1])
        )
    elif len(sys.argv) == 2 and (sys.argv[1].endswith(".yaml") or sys.argv[1].endswith(".yml")):
        gen_args, chunk_args, ret_args, rerank_args, read_args = parser.parse_yaml_file(
            json_file=os.path.abspath(sys.argv[1])
        )
    else:
        gen_args, chunk_args, ret_args, rerank_args, read_args = parser.parse_args_into_dataclasses()
